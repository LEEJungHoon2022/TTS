...
def load_model(hparams):
    ...
    if hparams.fp16_run: # use Multi-GPU
        model.decoder.attention_layer.score_mask_value = finfo('float16').min

    if hparams.distributed_run: # use Multi-GPU
        model = apply_gradient_allreduce(model)

    return model # tarcotron2 model 반환
...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus,
    ...
    """
    if hparams.distributed_run: # multi gpu 사용시 분산 실행
        init_distributed(hparams, n_gpus, rank, group_name)

    torch.manual_seed(hparams.seed) # seed 고정 1234
    torch.cuda.manual_seed(hparams.seed) # seed 고정 1234

    model = load_model(hparams) # 모델을 불러옴 위에 정의한 load_model이 불러와짐
    learning_rate = hparams.learning_rate # 정의한 lr
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-03, weight_decay=hparams.weight_decay)# otimizer 설정 Adam 사용(성능이 안정적임) decay는 lr 반올림??
    ...
    criterion = Tacotron2Loss() # loss function 정의
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output_directory', type=str,
                        help='directory to save checkpoints') # checkpoint 저장 경로
    parser.add_argument('-l', '--log_directory', type=str, 
                        help='directory to save tensorboard logs') # log 저장 경로
    parser.add_argument('-c', '--checkpoint_path', type=str, default=None, 
                        required=False, help='checkpoint path')
    parser.add_argument('--image_directory', type=str, default='images') # 
    parser.add_argument('--warm_start', action='store_true', # fine tuning에 사용할 모델 불러오기
    ...
    print("FP16 Run:", hparams.fp16_run) # multi gpu use
